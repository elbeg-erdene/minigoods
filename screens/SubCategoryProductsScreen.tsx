...
const SubCategoryProductsScreen: React.FC<SubCategoryProductsScreenProps> = ({ subCategory, parentCategoryName, products, onAddToCart, onProductClick, onBack }) => {
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark animate-in slide-in-from-right duration-300">
      <header className="sticky top-0 z-50 bg-white/95 dark:bg-background-dark/95 backdrop-blur-md px-4 py-4 flex items-center gap-3 border-b border-gray-100 dark:border-white/5">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-bold truncate">{subCategory.name}</h1>
          <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{parentCategoryName}</p>
        </div>
      </header>
      ...
      <div className="p-4">
        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 opacity-40">
            ...
            <p className="text-sm font-bold mt-2">Энэ ангилалд бараа байхгүй байна</p>
          </div>
        ) : (
